...
public class GameControl {
	...
	public static void main(String[] args) {
		...
		System.out.println("Count of " + gameLength + " seconds game ");
		System.out.println("Press any key and enter to start .. then estimate when " + gameLength + " seconds have elasped");
		sc.nextLine();
		...
		if (thread.isAlive()) {
			...
			if (timer.getCurrentCount() < gameLength) {
				System.out.println("Too early : that was " + timer.getCurrentCount() + " seconds");
			} else if (timer.getCurrentCount() > gameLength) {
				System.out.println("Too late : that was " + timer.getCurrentCount() + " seconds");
			} else {
				System.out.println("Nice! You got it exactly right!");
			}
			// stop the thread
			thread.interrupt();
		} else {
